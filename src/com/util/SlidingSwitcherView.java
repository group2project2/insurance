...
public class SlidingSwitcherView extends RelativeLayout {   // implements OnTouchListener
	...
	public SlidingSwitcherView(Context context, AttributeSet attrs) {
		super(context, attrs);
		TypedArray a = context.obtainStyledAttributes(attrs,com.insurance.R.styleable.SlidingSwitcherView);
		boolean isAutoPlay = a.getBoolean(com.insurance.R.styleable.SlidingSwitcherView_auto_play, false);
		if (isAutoPlay) {
			startAutoPlay();
		}
		a.recycle();
	...
	private void refreshDotsLayout() {
		...
		for (int i = 0; i < itemsCount; i++) {
			...
			if (i == currentItemIndex) {
				image.setBackgroundResource(com.insurance.R.drawable.dot_selected);
			} else {
				image.setBackgroundResource(com.insurance.R.drawable.dot_unselected);
			}
